...
export const fetchPoolsTotalStaking = async () => {
  const nonBnbPools = poolsConfig.filter((p) => p.stakingToken.symbol !== 'DEX')
  const bnbPool = poolsConfig.filter((p) => p.stakingToken.symbol === 'DEX')
...
export const fetchPoolsStakingLimits = async (
  ...
  const validPools = poolsConfig
    .filter((p) => p.stakingToken.symbol !== 'DEX' && !p.isFinished)
